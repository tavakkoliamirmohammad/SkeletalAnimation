    ...
    for (int i = 0; i < boneCount; i++) {
        ...
        for (int j = 0; j < segments; j++) {
            ...
            for (auto y : {5.f, -5.f}) {
                if (j < segments / 2) {
                    skin.push_back(
                            Vertex{x, y, 0, 0, previousBoneIndex, i, (0.5 - j / segments), (0.5 + j / segments)});
                } else {
                    skin.push_back(Vertex{x, y, 0, 0, i, nextBoneIndex, (1.0 - (j - segments / 2.0) / segments),
                                          ((j - segments / 2.0) / segments)});
                }
            }
        }
    }
